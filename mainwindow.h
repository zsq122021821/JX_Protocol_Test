...
private:
    ...
    QUdpSocket  *MudpSocket;//组播接收Socket
    QUdpSocket  *UudpSocket;//单播Socket
    QUdpSocket  *MudpSendSocket;//组播发送Socket
...
public:
    ...
    ~MainWindow();
void DispData(PROCESS_DATA_STRUCT *ptProcessData,QHostAddress targetAddr);
...
//自定义槽函数
    ...
    void on_set_clicked();
void onTimerOut();
    void on_TestItem_currentIndexChanged(int index);

   void on_stoptest_clicked();

   void on_actStartSend_triggered();

   void on_actStopSend_triggered();

   void on_BoardType_currentIndexChanged(int index);
